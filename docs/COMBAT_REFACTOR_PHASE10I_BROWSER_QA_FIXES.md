# Phase 10i — Browser QA fixes: Guard copy, guard motion, enemy impact FX, audio, BG wiring
...
- 至少提供兩個基本 presentation SFX：player impact 與 enemy impact。若 repo 沒有可重用 SFX binary，本批使用 WebAudio oscillator/noise-free synthetic one-shot，不新增外部資產、不改 core。
- Scene shutdown/destroy 必須停止 music 並釋放 presentation audio state。

## 新 BG 接線

- 使用者已提供 `area01-rail-halt-hd2d-candidate-v2.png`。
- 由於目前 GitHub contents connector 只能提交 UTF-8 text，branch 內已建立 q60 JPEG base64 chunks 作 temporary QA derivative。
...
- self guard destination test：不離開 actor current position。
- enemy impact plan 保持 target-aware。
...
- 背景確實是使用者提供的新 rail-halt hd2d candidate。
