...
- 2026-08-21 project consolidation: normalized shared work onto `main`; changed download sync to always fast-forward `origin/main`; extracted named-player asset queuing and encounter setup from `BootScene`; added a release asset audit and current project handoff; documented Rain Boss as blocked without an approved master; added AssetRecipe Draft 2020-12 schema, validator, verified route-map recipe and npm validation entry; separated Phaser into a cacheable vendor chunk. Balance simulation was explicitly skipped by user request.
- 2026-08-21 production verification for consolidation commit `ce6213e`: Vercel served the expected `index-C04w-awv.js`; the 1280x720 route map entered battle from the first available node and rendered all four player actors without black asset rectangles; the 844x390 landscape route remained fitted and readable. Local gates passed: asset metadata validation, 31 test files / 119 tests, production build and `git diff --check`.
- 2026-08-21 whole-project audit recorded in `docs/PROJECT_AUDIT_2026-08-21.md`: identified duplicate player texture pipelines, mixed source resolutions, stale/mojibake authoritative documents, an 82 KB compressed `BootScene`, 537 tracked delivery/tmp files, 258 identical-hash asset groups (about 68.6 MB redundant), missing scene/visual regression gates, and a canonical build that fails when the existing `dist` output is locked. Asset validation and 119 tests passed; a fresh-output production build passed. No art was generated, replaced or approved.
- 2026-08-21 audit remediation batch 1/2/3/6/7: replaced competing player pose keys with the single typed `player-assets.json` manifest and `player-*` runtime namespace; fourth roster member `mo` now resolves directly to Redleaf production PNGs; added deterministic player file/key/alpha/canvas validation; rewrote the active docs index, architecture, combat summary, Art Bible, character index/specs and area runtime status as clean UTF-8; moved canonical and Vercel output from locked legacy `dist/web` to `build/web`; extracted battle audio and shared loading-screen responsibilities from `BootScene`; added `qa-battle` direct encounter entry and a test covering setup for every route battle node. Verification: 32 unique pose keys, 31 test files / 120 tests, production build and `git diff --check` pass. Rin/Chikage/Oboro remain explicitly flagged as 128x128 prototype action sources; no art was generated or approved.
## 2026-08-21 — Formal encounter QA probe
...
## 2026-08-21 — P0/P1 Batch：規格對齊、固定 QA、站守實戰與路線終幕

- 清理目前權威文件中的過期 Boss／背景狀態；F1–F4 仍維持 `runtime-trial`，未擅自升級為 approved。
- 崩勢正式定案為首次歸零追加 4 HP、Balance 重置 8；crusher 厚甲、未授權威力罰則與掩護護符 9 已和現行 source/tests 對齊。
