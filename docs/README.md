...
29. `COMBAT_REFACTOR_PROGRESS.md`：重構分支目前完成／待驗證／deployment gate 狀態；只記進度，不取代規格與實作計畫。
30. `COMBAT_ACCEPTANCE_CHECKLIST.md`：現行戰鬥驗收條件；重構期間需逐批建立對應的新驗收項目，不可直接把舊交鋒／整輪規劃條件視為新系統完成標準。
31. `art-bible.md`：角色、怪物、背景與透明素材規範。
32. `characters/README.md`：角色母版與 runtime 對照。
33. `areas/README.md`：區域背景與 runtime 狀態。
34. `RELEASE_ASSET_AUDIT.md`：發布授權閘門。
35. `PLANNING_LOG.md`：採用決策與驗證紀錄。
36. `NEXT_WORK.md`：`main` 現行產品線的執行順序；`combat-refactor-v1` 不得把其中的舊戰鬥 UI／整輪規劃事項當成新玩法約束。
