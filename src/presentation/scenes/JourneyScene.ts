...
export class JourneyScene extends Phaser.Scene{
  ...
  private showNodePreview(node:StoryRouteNode){
    ...
    if(encounter){
      ...
      const battlefieldLabel=encounter.battlefield==='rail-halt'?'雨夜沿線月台':encounter.battlefield==='mountain-cut'?'山壁切通':encounter.battlefield==='forest-path'?'林間參道':encounter.battlefield==='rooftop'?'列車車頂':encounter.battlefield==='wayside'?'沿線停靠':'離車山道';
