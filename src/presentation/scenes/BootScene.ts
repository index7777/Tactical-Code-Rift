...
import{createDiscardProofDeckState}from'../../core/cards/BattleCards';
import{lowestHpTarget,RAIN_BOSS_MAX_BALANCE,RAIN_BOSS_MAX_HP,rainBossSkill}from'../../core/battle/BossRules';
...
private loadingLayer?:Phaser.GameObjects.Container;private discardProof=false;
...
// 第一區怪物皆走已接入的 PNG runtime；站守使用核准 Boss 母版。
...
// rain-boss 使用已核准的 v2 母版所輸出的 PNG runtime。
...
private returnToJourney(){if(this.journeyNodeId==='boss-1')this.registry.set('journey-area01-cleared',true);this.battleAudio?.fadeTo(0,350);this.time.delayedCall(350,()=>{this.battleAudio?.stop();this.scene.start('JourneyScene')})}
private hud(){
...
private rebuild(){
  ...
  const skills=new Map(es.map((e,i)=>{const skill=e.archetype==='rain-boss'?rainBossSkill(1):roundSkills[i]!,target=Phaser.Math.RND.pick(ps);return[e.id,{id:`${e.id}-skill`,...skill,targetId:target.id}]}));
  this.timeline=buildRoundTimeline(ps,es,skills);ps.forEach(f=>this.addActor(f));es.forEach(f=>this.addActor(f));this.deck=this.discardProof?createDiscardProofDeckState():refillHand(this.deck,5);
...
private addActor(f:Fighter){
    ...
    this.world.add(root);
    const maxHp=f.team==='player'?44:f.archetype==='rain-boss'?RAIN_BOSS_MAX_HP:40,maxBalance=f.archetype==='rain-boss'?RAIN_BOSS_MAX_BALANCE:8,a={root,x:p.x,y:p.y,sprite,hud,hudView,hit,hp:maxHp,maxHp,shield:0,tempShield:0,balance:maxBalance,alive:true,exposed:false,broken:false,archetype:f.archetype,traitReady:true};
...
private discardHandCard(card:BattleCard,fromX:number){
  ...
  this.discardMode=false;this.discardUsedThisRound=true;this.selected=undefined;this.visibleHandCount=Math.min(this.visibleHandCount,this.deck.hand.length);this.renderHand();this.animateCardTravel(fromX,310,0x823447);this.setStatus('已棄 1 張；下一回合補牌。');this.publishQaState()
}
...
private nextRound(automatic=false){
    ...
    const targets=[...this.players.entries()].filter(([,a])=>a.alive).map(([id])=>id),enemyNodes=this.timeline.filter(n=>n.team==='enemy'),roundRoles=enemyNodes.map(n=>(this.enemies.get(n.actorId)?.archetype??n.enemySkill?.archetype??'wet-corpse') as EnemyArchetype),roundSkills=dealEnemySkillsForArchetypes(roundRoles,Math.random,enemyNodes.map(n=>n.enemySkill?.name)),bossIsolated=[...this.enemies.values()].filter(a=>a.alive&&a.archetype!=='rain-boss').length===0,bossTarget=lowestHpTarget([...this.players.entries()].map(([id,a])=>({id,hp:a.hp,alive:a.alive})));
    ...
    for(const node of this.timeline){
      ...
      if(node.team==='enemy'){const generic=roundSkills[enemyIndex++]!,skill=role==='rain-boss'?rainBossSkill(this.round,bossIsolated):generic;node.enemySkill={id:`${node.actorId}-skill-r${this.round}`,...skill,targetId:role==='rain-boss'&&bossTarget?bossTarget:Phaser.Math.RND.pick(targets)};if(role==='rain-boss'){const boss=this.enemies.get(node.actorId);if(boss){boss.sprite.setTint(bossIsolated?0xffa3ad:0xffffff);boss.root.setData('bossPhase',bossIsolated?'isolated':'formation')}}}
    }
