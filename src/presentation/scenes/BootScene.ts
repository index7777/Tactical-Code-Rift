...
private previewTarget(id:string){
    ...
    if(this.selected.definitionId==='cover'){
      ...
      this.status.setText('掩護成立｜已截斷殺生線');
...
private skip(){const node=this.currentPlanner();if(this.busy||!node)return;const actorId=node.actorId;this.commands.set(node.id,null);this.skipBonusNext.add(actorId);this.planIndex++;this.renderTimeline();this.updateUndoVisibility();this.status.setText('跳過｜下回合速度 +2（一次）');this.focus()}
private undoCommand(){
