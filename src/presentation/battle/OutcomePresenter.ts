...
export class OutcomePresenter {
  ...
  async show(result:'victory'|'defeat',actions:{onRetry:()=>void;onContinue?:()=>void}){
    ...
    title.setScrollFactor(0);const retry=this.scene.add.text(layout.retry.x,layout.retry.y,'再戰',{fixedWidth:layout.retry.primary?240:190,align:'center',fontFamily:'sans-serif',fontSize:layout.retry.primary?'21px':'17px',color:'#fff',backgroundColor:layout.retry.primary?'#713141':'#35414d',padding:{y:layout.retry.primary?16:11}}).setOrigin(.5).setDepth(181).setAlpha(0).setScrollFactor(0).setInteractive({useHandCursor:true});retry.on('pointerdown',actions.onRetry);
