...
export class OutcomePresenter {
  ...
  async show(result:'victory'|'defeat',actions:{onRetry:()=>void;onContinue?:()=>void}){
    ...
    const retry=this.scene.add.text(victory?548:640,382,'再戰',{fixedWidth:160,align:'center',fontFamily:'sans-serif',fontSize:'18px',color:'#fff',backgroundColor:victory?'#35414d':'#713141',padding:{y:12}}).setOrigin(.5).setDepth(181).setAlpha(0).setInteractive({useHandCursor:true});retry.on('pointerdown',actions.onRetry);
    const buttons:Phaser.GameObjects.GameObject[]=[retry];
    if(victory&&actions.onContinue){const next=this.scene.add.text(732,382,'繼續旅程',{fixedWidth:180,align:'center',fontFamily:'sans-serif',fontSize:'18px',color:'#fff',backgroundColor:'#285c67',padding:{y:12}}).setOrigin(.5).setDepth(181).setAlpha(0).setInteractive({useHandCursor:true});next.on('pointerdown',actions.onContinue);buttons.push(next)}
