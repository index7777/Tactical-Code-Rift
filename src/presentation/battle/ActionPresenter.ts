...
export class ActionPresenter {
  ...
  async relay(sourceId: string, allyId: string, targetId: string, enemy = false) {
    ...
    const badge = this.scene.add.text(contactX, target.root.y - 105, '接力', { fontFamily: 'sans-serif', fontSize: '18px', fontStyle: 'bold', align: 'center', color: '#fff', backgroundColor: enemy?'#713142':'#72511c', padding: { x: 16, y: 8 } }).setOrigin(.5).setDepth(90);this.combatLayer.add(badge);
    ...
    await this.move(target.root, target.x, target.y, 145, 'Back.easeOut');
    await this.wait(80); badge.destroy();
  ...
  async dualRelay(playerSourceId:string,playerAllyId:string,enemySourceId:string,enemyAllyId:string){
    ...
    const badge=this.scene.add.text(640,y-112,'雙方接力',{fontFamily:'sans-serif',fontSize:'18px',fontStyle:'bold',color:'#fff',backgroundColor:'#4f304fee',padding:{x:16,y:8}}).setOrigin(.5).setDepth(95);this.combatLayer.add(badge);
    await Promise.all([
    ...
    badge.destroy();await Promise.all([this.move(pa.root,pa.x,pa.y,230),this.move(ps.root,ps.x,ps.y,230),this.move(ea.root,ea.x,ea.y,230),this.move(es.root,es.x,es.y,230)])
  }
