...
export class BattlefieldPresenter{
  ...
  build(mode:BattlefieldMode){
    ...
    }else if(mode==='terminal-platform'){
      add(this.scene.add.image(640,360,'bg-world01-terminal-platform-trial').setDisplaySize(1280,720));
  ...
  private addRuntimeForeground(root:Phaser.GameObjects.Container,mode:BattlefieldMode){
    ...
    if(mode==='wayside'||mode==='rail-halt'||mode==='mountain-cut'||mode==='terminal-platform')image(1190,495,'battle-fg-rail-debris',178,74,.22);
