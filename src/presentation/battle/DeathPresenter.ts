...
export class DeathPresenter {
  ...
  play(actor:VisualActor&{hud?:Phaser.GameObjects.Container},enemy:boolean,style:DeathStyle='normal'){
    // Death owns the actor from this point onward. Idle/bobbing animation must
    // not continue underneath the collapse pose.
    const heroine=Boolean(actor.sprite?.getData('heroine'));
    if(actor.sprite){actor.sprite.anims.stop();this.scene.tweens.killTweensOf(actor.sprite);actor.sprite.setAngle(0);if(heroine)playHeroinePose(actor.sprite,'down')}
    const color=enemy?0xff536f:0x8eeeff,scale=style==='heavy'?1.7:style==='relay'?1.45:1.2,exit=deathExitPlan(enemy,style);
    ...
    this.scene.time.delayedCall(exit.impact,()=>{
      ...
      this.scene.tweens.add({targets,alpha:exit.removeFromField?0:.18,duration:exit.fade,ease:'Cubic.easeIn',onComplete:()=>{
        if(exit.removeFromField){actor.root.setVisible(false);actor.hud?.setVisible(false)}
      }})
    });
  }
}
