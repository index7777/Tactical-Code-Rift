...
export class FighterHudPresenter{
  ...
  create():FighterHudView{
    ...
    // P11.3: compact telemetry with exact 0–8 posture segmentation.
  ...
  refresh(view:FighterHudView,state:FighterHudState,animate=true){
    ...
    const maxHp=state.maxHp??100,hpRatio=Phaser.Math.Clamp(state.hp,0,maxHp)/maxHp,targetWidth=54*hpRatio;this.scene.tweens.killTweensOf(view.hpEcho);view.hpFill.width=targetWidth;view.hpFill.setFillStyle(hpRatio<=.25?0xff5a6d:0xd94c61,1);
