...
export function resolveMonsterClashPower(archetype:EnemyArchetype|undefined,card:Pick<BattleCard,'definitionId'|'clashPower'|'tempo'>){if(archetype==='swift'&&card.tempo<=0)return Math.max(0,card.clashPower-2);if(archetype==='hexer'&&card.clashPower>=7&&card.definitionId!=='break'&&card.definitionId!=='delay')return Math.max(0,card.clashPower-2);return card.clashPower}
...
export function resolveMonsterHit(archetype:EnemyArchetype|undefined,card:Pick<BattleCard,'definitionId'|'clashPower'|'tempo'|'damage'|'balanceDamage'>,context:MonsterHitContext):MonsterHitResult{
  ...
  if(archetype==='crusher'&&context.traitReady){if(card.definitionId==='break'){balanceDamage+=2;cue='stone-guard'}else{damage=Math.max(1,damage-5);consumeTrait=true;cue='stone-guard'}}
