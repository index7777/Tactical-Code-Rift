...
describe('relay continuation planning',()=>{
  it('attaches a later relay card to a tie on the same enemy',()=>{
    ...
    const pa:ActionNode={id:'PA-action',team:'player',actorId:'PA',actorIndex:0,speed:8,order:0};
    const pb:ActionNode={id:'PB-action',team:'player',actorId:'PB',actorIndex:1,speed:6,order:2};
    const ea:ActionNode={id:'EA-action',team:'enemy',actorId:'EA',actorIndex:0,speed:7,order:1,enemySkill:{id:'EA-skill',name:'牽制',clashPower:6,damage:5,targetId:'PA'}};
    const commands=new Map<string,PlayerCommand|null>([
      [pa.id,{nodeId:pa.id,actorId:'PA',card:breakCard,targetNodeId:ea.id,targetActorId:'EA'}],
      [pb.id,{nodeId:pb.id,actorId:'PB',card:relayCard,targetNodeId:ea.id,targetActorId:'EA'}],
    ]);
    const beats=resolveBattleBeats([pa,ea,pb],commands),links=planPlayerRelayContinuations(beats);
    expect(beats[0]?.kind).toBe('clash');
    expect(links.get(ea.id)?.actorId).toBe('PB');
  });
