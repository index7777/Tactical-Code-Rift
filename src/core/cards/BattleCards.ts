...
export function createDiscardProofDeckState():TeamDeckState{const deck=createTeamDeck(),wanted:CardFamily[]=['quick','break','guard','cover','cycle'],hand=wanted.map(id=>deck.find(card=>card.definitionId===id)!),ids=new Set(hand.map(card=>card.instanceId));return{drawPile:deck.filter(card=>!ids.has(card.instanceId)),discardPile:[],exhaustPile:[],hand}}
